...
export type RunelightAnalysisResult = {
  entry: string
...
export function createRunelightAnalysisCache(sourceFilesByPath = new Map<string, ts.SourceFile>()): RunelightAnalysisCache {
  return {
...
function bindImportedStaticValuesForFile(
  ...
  cache: RunelightAnalysisCache | undefined,
  visited: Set<string>,
) {
  for (const statement of sourceFile.statements) {
    if (!ts.isImportDeclaration(statement) || !statement.importClause || !ts.isStringLiteral(statement.moduleSpecifier)) continue
