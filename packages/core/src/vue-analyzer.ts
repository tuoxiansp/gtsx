...
export function analyzeVueEntry(options: { cache?: RunelightAnalysisCache; cwd: string; entry: string }): RunelightAnalysisResult {
  ...
  const providerFrames: Record<string, RunelightProviderSummary> = Object.fromEntries(
    extraction.frameSourceFile ? getGVueProviderSummariesForFile(extraction.frameSourceFile, entryPath, options.cwd, options.cache) : [],
  )
...
function readProviderSelections(frameValue: ts.ObjectLiteralExpression): string[] | undefined {
  const providersProperty = frameValue.properties.find(
    (property): property is ts.PropertyAssignment =>
      ts.isPropertyAssignment(property) && getStaticPropertyName(property.name) === "provide",
  ) ?? frameValue.properties.find(
    (property): property is ts.PropertyAssignment =>
...
function readVueFrameStaticFacts(
  ...
  if (ts.isObjectLiteralExpression(frameValue)) {
    ...
    const provide = objectLiteralPropertyExpression(frameValue, "provide") ?? objectLiteralPropertyExpression(frameValue, "providers")
    if (provide) flattenVueProvideStaticValues(provide, values)
  }
...
function flattenVueProvideStaticValues(expression: ts.Expression, values: Map<string, VueStaticBranchValue>) {
  const provideValue = unwrapExpression(expression)
  if (!ts.isArrayLiteralExpression(provideValue)) return

  for (const element of provideValue.elements) {
    const entry = unwrapExpression(element)
    if (!ts.isArrayLiteralExpression(entry)) continue
...
function readProviderVariantMarkersFromType(typeNode: ts.TypeNode): Record<string, RunelightProviderVariantSelection> {
  ...
  function visit(node: ts.TypeNode) {
    ...
    if (node.typeName.text !== "GProviderFrame" && node.typeName.text !== "GVueProvideFrame") return
...
function sourceFileForAbsolutePath(filePath: string, cache?: RunelightAnalysisCache): ts.SourceFile | undefined {
