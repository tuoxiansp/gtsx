...
describe("Runelight React transform", () => {
  it("wraps default component exports that declare frames", () => {
    const output = transformRunelightComponentBoundaries({
      root,
...
`,
    ...
    expect(output).toContain('import { defineGComponent as __runelightDefineGComponent } from "@runelight/core"')
...
`,
    ...
    expect(output.startsWith('"use client"\nimport { defineGComponent as __runelightDefineGComponent } from "@runelight/core"')).toBe(true)
  })
...
`
    ...
    expect(output).not.toContain("@runelight/core")
