...
describe("Runelight Vue support", () => {
  ...
  it("preserves <g:frames> imports so Vue provide keys exist at preview runtime", () => {
    const transformed = transformRunelightVuePreviewSfc(
      [
        "<template>",
        ...
        "import { authKey } from './auth'",
        "export default {",
        "  admin: { props: {}, provide: [[authKey, { role: 'admin' }]] },",
        "}",
        "</g:frames>",
      ].join("\n"),
      "src/UserCard.g.vue",
    ...
    expect(transformed).toContain("provide: [[authKey, { role: 'admin' }]]")
  })
