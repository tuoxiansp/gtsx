...
describe("Runelight Vue preview route params", () => {
  it("reads preview target params from URL search params", () => {
    const params = readRunelightVuePreviewRouteParams(
      new URLSearchParams(
        "entry=src/UserCard.g.vue&frame=ready&chrome=0&pool=1&static=1&sessionId=session_1&frameOverride=userId:user_1&frameOverride=src%252FChild.g.vue%2523default%3Aopen%253Aerror&inputOverride=src%252FToast.g.vue%2523default%3Atop",
      ),
    )
    ...
    expect(params.inputOverrides).toEqual(
      new Map([["src/Toast.g.vue#default", "top"]]),
    )
  })
})
