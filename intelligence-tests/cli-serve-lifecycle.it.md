...
- `runelight --help` and the relevant command help expose the current public CLI surface for `check`, `inspect`, `preview-targets`, `containing-frames`, `changes`, `serve`, and `capture`, and do not advertise commands that are not implemented.
