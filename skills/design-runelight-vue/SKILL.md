---
name: design-runelight-vue
description: Legacy/internal optional workflow for Vue Runelight design drafts in project.entryRoot/design/*.g.vue. Use only when the user explicitly asks to continue design-draft work or names this skill; setup does not install it by default.
---

# Design Runelight Vue

Use this framework-specific skill only for explicit legacy/internal design-draft work inside a Vue Runelight workspace. Setup does not install this skill by default; for the current recommended visual-improvement path, create `.g.vue` coverage with authoring/refactor skills and use `polish`.
...
The user talks to the local agent. The agent edits local Vue design frames. Studio renders the design board.
...
3. Agent creates or updates one or more `.g.vue` design frames under `project.entryRoot/design`.
4. User opens the Studio design workspace, compares frames, gives feedback, and iterates.

## File Contract

- Put Vue design drafts in `project.entryRoot/design/<FrameName>.g.vue`, where `project.entryRoot` comes from `runelight.config.ts`.
...
- Each design file should contain one ordinary Vue SFC default component and one `<g:frames>` block with a design frame named `live`.
- Multiple alternatives are multiple files, not multiple frame keys. Use names such as `CheckoutFlowCalm.g.vue`, `CheckoutFlowDense.g.vue`, and `CheckoutFlowEditorial.g.vue`.
- Prefer self-contained Vue SFCs. Import existing design-system CSS, tokens, or simple presentational components only when they are stable in preview.
...
`DESIGN_REFERENCE.md` is an aesthetic reference only. Use it for taste, hierarchy, layout discipline, anti-default rules, accessibility checks, and visual critique. It does not override the Vue project, the existing host, or the Runelight frame contract.
...
When the template needs sample data, make it frame-visible:
...
export default {
  live: {
    ...
    scope: {
      title: "Gift checkout",
      summary: "A calm confirmation moment before sending.",
    },
  },
}
</g:frames>
...
5. **Design reference pass**: for substantive visual work, read [`DESIGN_REFERENCE.md`](./DESIGN_REFERENCE.md), choose a concrete visual direction, and apply the relevant surface rules. Use it as an aesthetic reference, not as a substitute for product context, the existing Vue host, or the Runelight frame contract.
6. **Visual/layout plan**: decide the visual anchor, aesthetic family, information hierarchy, primary action, secondary actions, data density, and visual system before writing the SFC.
7. **Frame drafting**: create or update named `.g.vue` design frames with credible content and stable dimensions.
...
- If the user asks for broad exploration, create 2-3 named `.g.vue` files with distinct concepts.
...
If Studio does not show the frame, stop and report that the design workspace is unavailable. Common causes are missing `project.entryRoot`, stale preview module globs, or a dev server that needs restart after adding the first design frame.
...
- The `.g.vue` design frames created or changed.
- What was checked.
