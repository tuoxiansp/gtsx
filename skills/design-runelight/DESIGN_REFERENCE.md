...
### 4.6 Information Diet, Data & Form Patterns
* **MINIMUM VIABLE INFORMATION (mandatory):** Build the first design pass from the fewest visible facts that communicate the product moment. Start with one primary message, one supporting detail, and one action or outcome. Add metadata only when it changes what the user understands, decides, or does.
* **DELETE BEFORE DESIGNING:** Before styling a section, remove every label, stat, badge, timestamp, avatar, caption, and helper line that is only decorative, redundant, or "nice context." If the design only works after adding extra microcopy everywhere, the hierarchy is weak.
* **PROGRESSIVE DISCLOSURE OVER INFO PACKING:** Secondary details should live behind hover, tabs, disclosure, carousel depth, or the next screen when they are not needed for the immediate read. Do not solve uncertainty by showing everything at once.
