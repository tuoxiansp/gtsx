...
This reference is copied from `Leonxlnx/taste-skill` and is intentionally kept close to the upstream image-to-code skill. In `design-gtsx`, interpret "website", "frontend", "implementation", and "code" as a gtsx design-frame implementation unless the user explicitly asks to modify the production app.

The implementation target is `project.root/gtsx/design/<FrameName>.g.tsx`: one self-contained default-exported React component with one happy-path frame named `live`. The image remains the visual source of truth; the `.g.tsx` frame is the translation layer rendered in Studio.

## gtsx asset translation boundary

During image analysis, classify every visible element before coding:

- **Structural GUI**: layout, text, controls, cards, navigation, forms, tables, lists, charts, spacing, colors, borders, shadows, and repeatable components. Rebuild these in TSX/CSS/SVG.
- **Bounded asset**: photography, product renders, editorial images, UI crops, complex 3D, illustration, texture, or brand artwork. Keep these only as explicit media slots or saved image assets when the environment supports them.
- **Simplified effect**: complex lighting, particles, depth, material, or texture that can be approximated with gradients, shadows, masks, simple SVG, CSS filters, or subtle background treatment.

Do not pretend that `.g.tsx` can faithfully recreate raster-only artwork. If a generated reference depends on a photo shoot, complex 3D render, intricate illustration, custom logo, dense texture, or cut-out asset, either preserve it as a bounded asset or regenerate a more GUI-reconstructable reference before implementing.
