...
This reference is copied from `Leonxlnx/taste-skill` and is intentionally kept close to the upstream image-generation skill. In `design-gtsx`, use it only when the current agent environment has image generation capability.

Use the generated images as visual source material for gtsx design frames. After generating the section/screen images, continue with [`IMAGE_TO_GTSX_REFERENCE.md`](./IMAGE_TO_GTSX_REFERENCE.md) and translate the image system into `project.root/gtsx/design/*.g.tsx`.

## gtsx renderability boundary

In `design-gtsx`, generated images are web GUI references, not finished raster comps. The image should define a visual system that can be rebuilt with React, HTML, CSS, SVG, canvas, and optional bounded image assets.

Generate designs whose primary structure is based on:

- Web layout primitives: sections, nav bars, sidebars, grids, flex rows, columns, split views, cards, lists, tables, forms, buttons, tabs, drawers, modals, pricing blocks, dashboards, charts, timelines, galleries, and content modules.
- Editable interface content: real text, labels, numbers, controls, icons, states, hierarchy, spacing, and component relationships that can become TSX.
- CSS/SVG-friendly visual language: color fields, gradients, borders, shadows, radius, masks, dividers, simple geometry, line work, simple patterns, subtle noise, and repeatable decorative motifs.
- Bounded media slots: hero media, product images, editorial photos, UI crops, framed objects, or simple rendered assets placed inside clear frames or background slots with stable aspect ratios.
- System cues: palette, typography scale, spacing rhythm, image treatment, component styling, and reusable layout rules.

Do not generate references whose value depends mainly on raster-only content that a GUI language cannot reasonably reconstruct:

- Photorealistic scenes, people, fashion/editorial shoots, interiors, landscapes, food, vehicles, or products where the exact image content must be preserved.
- Complex 3D renders, isometric worlds, character art, mascots, game art, cinematic VFX, particles, fluid effects, volumetric lighting, or impossible glass/refraction effects.
- Intricate illustration, collage, painterly art, dense texture systems, hand lettering, poster art, custom logos, or brand marks that would need to be cut out as assets.
- Full-page bitmap compositions where layout, text, and components are fused into one image instead of separable UI structure.
- Tiny unreadable screenshots, dense charts, microscopic dashboards, or visual details that cannot be extracted into editable TSX.
- Any design that collapses if the raster artwork is replaced by a simpler asset, placeholder, gradient, or media frame.

When the brief asks for a complex visual asset, choose the appropriate route before rendering:

1. **Asset route**: if the environment can save and reference generated images, keep the complex visual as an explicit bounded asset and design the surrounding GUI as codeable structure.
2. **GUI route**: if assets cannot be saved or embedded, regenerate the concept as CSS/SVG-friendly GUI: simpler geometry, bounded media frames, editable text, and reproducible layout.
3. **Hybrid route**: use one or two bounded raster assets for atmosphere or product presence, while all navigation, copy, CTAs, cards, data, and layout remain rebuildable in `.g.tsx`.

Before rendering, check: can the result be translated into a self-contained `.g.tsx` frame without pretending CSS can recreate a photo shoot, complex 3D object, illustration, or cut-out asset? If not, simplify the art direction or make the raster element an explicit bounded media slot.
