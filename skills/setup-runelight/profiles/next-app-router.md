...
- Use `runelightNextReact()` without statically importing the Runelight config from `next.config.*`; the adapter loads the Runelight config only when Runelight dev preview is active.
- `.g.tsx` files are production React components. Do not move normal app imports away from `.g.tsx`; isolate only preview routes, the session route, adapter-generated Runelight files, and config loading from production.
- The Next preview/session integration is development-only. Normal setup must not change production app behavior, expose usable `/runelight*` production routes, or read/write `${project.entryRoot}/.runelight/` at production runtime.
- The adapter writes generated Runelight files under `${project.entryRoot}/.runelight/`, then wires webpack/Turbopack for preview imports when Runelight dev preview is active. Ensure `.gitignore` contains `.runelight/`; that single directory pattern covers this generated folder at any depth. For `src/app` projects, set `project.entryRoot: "src/app/runelight"` so generated files stay inside `src` without extra adapter output configuration. Do not add user-authored `.runelight` imports or a custom `.g.tsx` Turbopack loader in app code.
- Record the local Runelight route entry directory in `project.entryRoot`.
...
- After package upgrades, rerun Next.js typecheck/dev verification. If adapter exports, route helper signatures, adapter-generated file wiring, or preview route behavior changed, migrate only the affected glue while preserving existing route isolation and config-wrapper composition.
...
`runelight serve` substitutes `{port}` with the Runelight-owned port, sets `RUNELIGHT_DEV=1`, waits for `/runelight/session`, and prints the serve URL plus the base `/runelight` preview URL. User-facing routes are fixed and not configurable: `/runelight` and `/runelight/session`.

## Route Files

### App Router Layout Boundary Audit And Remediation

Before adding `${project.entryRoot}/*`, audit and clean the route layout chain that will wrap `/runelight` and `/runelight/session`. Treat this as a setup gate, not a browser-only verification step.
...
The preview/session route is clean only if the inherited layout chain is limited to static document and visual setup: `<html>`, `<body>`, global CSS imports, font classes, static theme classes or `data-*` attributes, metadata, and non-hook bootstrap scripts.

If the root layout contains production shell behavior, remove the risk before claiming setup is complete. Do not hide it in `${project.entryRoot}/page.tsx`, and do not rely on runtime guards inside preview/session. The supported App Router remediation is to isolate layouts with route groups.
...
app/runelight/session/route.ts   # Runelight serve-session identity
```
...
`${project.entryRoot}/session/route.ts`:

```ts
import { createRunelightNextSessionResponse } from "@runelight/adapter-next-react/session-route"

export const dynamic = "force-dynamic"

export async function GET() {
  return createRunelightNextSessionResponse()
}
```
...
9. Confirm an existing app route still renders.
