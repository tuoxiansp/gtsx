...
10. Keep JSX-producing branches inspectable: direct conditionals over props/scope/providers, `if` returns, `&&`, `||`, and traceable `map`/render callbacks. Refactor helper predicates, `switch`, JSX-returning loops, and stored JSX variables before calling the refactor done.
11. Update imports from `./Component` to `./Component.g`. Preserve barrels.
12. Run `runelight check` + project typecheck. Render a frame in preview if available.
