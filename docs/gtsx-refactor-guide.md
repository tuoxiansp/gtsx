...
The canonical workflow for AI agents lives in [`skills/refactor-to-gtsx/SKILL.md`](../skills/refactor-to-gtsx/SKILL.md). This guide is the human-readable version: the decision framework, the key moves, and the traps to avoid.

If the project isn't wired for gtsx yet, run [`setup-gtsx`](../skills/setup-gtsx/SKILL.md) first. For authoring patterns in the resulting `.g.tsx`, see the [Authoring Guide](./gtsx-authoring-guide.md).
...
- **Runtime wrapper:** `<GtsxPreviewRuntime><OldClient /></GtsxPreviewRuntime>`
