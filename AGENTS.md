...
- Secondary audience: Human developers evaluating the product direction, maintaining the CLI, adapters, and preview/capture loop, or running examples and intelligence tests.
- Core artifact responsibility: Keep the tooling contract, `.g` protocol, static frame contract, preview sidecar workflow, framework implementation ends, setup/authoring/refactor/polish skills, examples, docs, tests, and product vocabulary coherent with each other.
- Main current risk: Cross-surface contract drift: an agent makes a locally reasonable change in one surface, but leaves the protocol, checker, preview index, adapters, skills, examples, tests, or product language inconsistent.
- This project is not trying to be: A generic component preview clone, a lowest-common-denominator framework abstraction, a replacement framework/runtime, a mature contributor-policy template, or a production-exposed Runelight surface by default.
...
- Prioritize: The needs of `runelight check`, preview targets, capture, session endpoints, frame enumeration, and static analysis.
...
- Reject: Changes that let one framework adapter's convenience rename, dilute, or distort protocol concepts such as frames, scope, providers, preview indexes, Host, Adapter, sidecar, or seam.
- Applies to: Protocol docs, analyzer/check behavior, preview index shape, preview transforms, adapters, skills, examples, and tests.
...
- Prioritize: Source shapes that `runelight check`, preview indexes, and preview transforms can statically inspect.
...
- Prioritize: Preview and capture as a sidecar hosted by the user's existing framework, with one explicit preview seam.
...
- Applies to: CLI serve/capture, Vite adapters, Next adapter, preview runtimes, preview route behavior, and setup profiles.
...
- Even though: Production Runelight exposure can be useful for controlled fixtures and demos.
...
- Reject: Documentation or setup changes that turn production Runelight preview assets into normal user guidance.
...
- Applies to: `skills/setup-runelight`, authoring/refactor/polish skills, README installer prompt, intelligence tests, and docs links.
- Micro-decisions affected: Whether to update a skill with a workflow change, which companion skill to install, how to describe polish references, and whether a test should assert skill behavior.
...
- Because: Wrapper frames hide the actual visual branch surface and weaken preview targets as a map of real UI states.
- Reject: Bulk converting route glue, provider nesting, layout slots, forwarding components, or scope values that contain React/Vue nodes just to get files into preview.
...
### Rule 9: No Design Draft Workspace

- Prioritize: Real `.g.tsx` and `.g.vue` coverage under the configured source root as the only Runelight visual source surface.
- Even though: A special scratch workspace under `project.entryRoot/design` can feel convenient for product exploration.
- Because: The current product goal is the polish loop over inspectable coverage, not a parallel design-draft capability.
- Reject: Creating or teaching `project.entryRoot/design`, `designRoot` config, placeholder frames during setup, serialized DOM, screenshots, browser layout positions, or design drafts as Runelight source artifacts.
- Applies to: setup profiles, examples, adapters, CLI discovery, intelligence tests, and polish skills.
- Micro-decisions affected: File placement, preview globs, generated registry discovery, watcher roots, change detection scope, and what verification URL to use.
- Exception: None by default; design exploration as a product capability must be confirmed separately before adding a new source surface.
...
- Prioritize: Runelight as a dense, scannable, state-oriented polish loop, and Runelight-facing visuals that show real product behavior with a strong point of view.
...
- Applies to: `packages/react`, `packages/vue`, `apps/website`, preview `.g.tsx` and `.g.vue` components, polish references, and capture assets.
- Micro-decisions affected: Color/token reuse, component density, iframe sizing, error states, visual proof assets, website tone, and whether a design change needs screenshot/preview verification.
- Exception: None for Runelight source surfaces. Exploratory visual work may happen in normal `.g` coverage, website frames, or external prototypes, but must not introduce a design workspace.
...
- Document what agents must do now: install/refresh the right project-local skills, preserve host behavior, run `runelight check`, verify session endpoints and preview URLs, and respect production defaults.
- Delay speculative docs for new implementation ends, public production preview exposure, or generalized host runtimes until the implementation and tests make those contracts real.
...
- Treat generated and adapter-owned files as generated. Do not store user source artifacts or runtime state under `.runelight`.
...
- UI is applicable: the repo contains preview runtimes, the website, Runelight captures, and polish skills.
- Preview and capture output should remain technical: dense, legible, canvas/iframe/state oriented, and optimized for scanning frames, variants, errors, boundaries, and render status.
...
- Reuse existing visual language before introducing colors, shadows, spacing, radii, or motion. The website currently uses a dark product-forward palette with cyan accent and real preview captures; preview code has precise layout constants for frame grids/chrome.
...
- Verify substantial UI work through preview/capture, or browser screenshots when practical, especially when iframe framing, canvas geometry, or responsive text can break the product experience.

### Product / UX

- Current product decisions should optimize for trustworthy install/setup, inspectable visual states, accurate preview rendering, and coherent product vocabulary over a mature public onboarding funnel.
- Expose limitations plainly: no production Runelight preview by default, no proof that every possible state combination is covered, no replacement for the user's app runtime, and no compatibility promise while internal pre-release.
...
- Agents should present a plan before cross-surface contract migrations, analyzer/transform rewrites, setup profile migrations, preview render scheduling changes, or significant website/design direction changes.
- Agents must ask a human before changing product boundaries: production Runelight preview as a formal feature, new framework implementation ends, new user-facing promises, new setup installation strategy, new protocol concepts, public CLI/config/export expansion, or route semantic changes.
...
- When verification is possible, prefer concrete project commands such as targeted `vitest`, `pnpm --filter ... typecheck`, `runelight check`, `runelight serve`, or browser/preview checks over claiming confidence from static reading alone.
...
1. What contract surface does this task touch?
   Options include tooling contract, .g protocol, analyzer/static checks, React implementation end, Vue implementation end, setup skills, adapter glue, CLI lifecycle, preview/capture UI, website, examples, docs, or intelligence tests.
...
7. What is the smallest verification that proves the intended contract?
   Pick targeted unit tests, typecheck, runelight check, session/preview checks, intelligence-test validation, or document-only review as appropriate.
```
...
- Do not refactor analyzer, transform, adapter, or preview scheduling code broadly unless a reproduced bug or contract change demands it.
- Do not formalize production Runelight exposure unless the task is explicitly about that product direction and preserves the default unexposed behavior until changed deliberately.
...
- Do not store design drafts, screenshots, serialized DOM, runtime values, or generated Runelight files as hand-authored repo artifacts.
...
- Do not make preview or website UI generic unless the change improves real product comprehension, frame inspection, or visual proof.
